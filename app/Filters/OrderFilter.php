...
class OrderFilter extends ApiFilter{
    ...
    public function queryFullDataOrder(){
       ...
       // DB::statement("SET SQL_MODE=''"); //add this line or change in config/database.php mysql strict to false
        ...
        }])->addSelect(['amount' => OrderDetail::join('products','order_details.product_id','=','products.id')->selectRaw('SUM(order_details.quantity * products.price)')->whereColumn('order_id','orders.id') ])->get();
